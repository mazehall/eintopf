﻿/**
 * original from https://github.com/vtortola/ng-terminal-emulator
 * date: 06.11.15
 *
...
.controller('terminalController', ['$scope', 'terminalConfiguration', 'promptCreator', function ($scope, terminalConfiguration, promptCreator) {
    ...
    $scope.init = function (configName) {
        ...
        $scope.inputOnlyMode = config.inputOnlyMode;
        $scope.secretSign = config.secretSign;
    ...
    $scope.handlePaste = function (e) {
        clipbardData = e.clipboardData.getData('text/plain');

        // replace clipboardData with secret signs when in secret mode
        if (!$scope.secret) {
            $scope.commandLinePrint += clipbardData;
        } else {
            $scope.commandLinePrint += new Array(clipbardData.length + 1).join( $scope.secretSign );
        }
    ...
    $scope.keypress= function (keyCode) {
        if ((!$scope.inputOnlyMode || $scope.input) && $scope.commandLine.length < 80) {
            commandIndex = -1;
            $scope.commandLine += String.fromCharCode(keyCode);
            if($scope.secret) { // show something different when secret
                $scope.commandLinePrint += $scope.secretSign
            } else {
                $scope.commandLinePrint += String.fromCharCode(keyCode);
            }
    ...
    $scope.previousCommand = function () {
        ...
        $scope.commandLinePrint = $scope.commandLine;
        $scope.$$phase || $scope.$apply();
    }
    ...
    $scope.execute = function () {
        ...
        var commandPrint = $scope.commandLinePrint;
        ...
        // don't add secrets in the history
        if(!$scope.secret && command != commandHistory[commandHistory.length-1])
            commandHistory.push(command);

        if (!$scope.inputOnlyMode || !$scope.input) {
            $scope.results.push({ type: 'text', text:[$scope.prompt.text + commandPrint] });
        }

        $scope.$emit('terminal-input', [{ command: command }]);

        if (!$scope.inputOnlyMode || !$scope.input) $scope.$apply();
    };

    $scope.backspace = function () {
        if ($scope.commandLine) {
            $scope.commandLine = $scope.commandLine.substring(0, $scope.commandLine.length - 1);
            $scope.commandLinePrint = $scope.commandLinePrint.substring(0, $scope.commandLinePrint.length - 1);
            $scope.$apply();
        }
    }
}])
...
.directive('terminal', function ($document) {
    return {
        ...
        transclude: true,
        replace:true,
        template: "<section class='terminal' ng-paste='handlePaste($event)'><div class='terminal-viewport'><div class='terminal-results'></div><span class='terminal-prompt' ng-show='showPrompt'>{{prompt.text}}</span><span class='terminal-input'>{{commandLinePrint}}</span><span class='terminal-cursor' ng-show='showCursor'>_</span><input type='text' ng-model='commandLine' class='terminal-target'/></div><div ng-transclude></div></section>",
        compile: function compile(tElement, tAttrs, transclude) {
            return {
                ...
                post: function postLink(scope, element, attrs, controller) { 
                        ...
                        consoleView.on('click', function (e) {
                            // disable focus in input only mode without active input
                            if (scope.inputOnlyMode && !scope.input) {
                        ...
                        scope.$watchCollection(function () { return scope.results; }, function (newValues, oldValues) {
                            ...
                            if (scope.inputOnlyMode) scope.showCursor = false;
                            var f = [function () {
                                if (!scope.inputOnlyMode || scope.input) {
                                    scope.showPrompt = true;
                                    scope.showCursor = true;
                            ...
                            for (var j = 0; j < newValues.length; j++) {
                                ...
                                if (scope.outputDelay) {
                                    ...
                                    // add cursor on the last element on input only mode
                                    if(scope.inputOnlyMode && scope.input) {
                                        results.append(prompt);
                                        results.append(consoleInput);

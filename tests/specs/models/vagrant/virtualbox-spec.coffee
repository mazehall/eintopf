...
describe 'virtualbox', ->
  ...
  describe "getOnlyVirtualBoxDir", ->
    ...
    it 'should fail with specific error when findAsync returns empty array', (done) ->
      model.__get__('jetpack.findAsync').andCallFake -> return fromPromise []

      model.getOnlyVirtualBoxDir (err) ->
        expect(err).toBeTruthy()
        expect(err.message).toEqual('Vagrant machines not found')
        done()

    it 'should fail with specific error when findAsync returns more than one result', (done) ->
      model.__get__('jetpack.findAsync').andCallFake -> return fromPromise [{absolutePath: samples.path}, {absolutePath: samples.path}]

      model.getOnlyVirtualBoxDir (err) ->
        expect(err).toBeTruthy()
        expect(err.message).toEqual("can't maintain integrity with multiple machine folders")
        done()
...
#        done()
  ...
  describe "getGuestStatus", ->
    ...
    beforeEach ->
      spyOn(model, 'runVBoxCmd').andCallFake (cmd, callback) ->
        process.nextTick -> callback null, '"vm1" {das213....}\r\n"vm2" {sa213....}\r\n"' + samples.vmInfoMapped.name + '" {8646....}\r\n"vm3" {1231dsa....}'

      spyOn(model, 'getOnlyVirtualBoxDir').andCallFake (callback) ->
        process.nextTick -> callback null, {absolutePath: samples.path, name: samples.vmInfoMapped.name }

    it 'should return true', (done) ->
      model.getGuestStatus (err, result) ->
        expect(err).toBeNull();
        expect(result).toBeTruthy();
        done()

    it 'should return false', (done) ->
      model.runVBoxCmd.andCallFake (cmd, callback) ->
        process.nextTick -> callback null, '"vm1" {das213....}\r\n"vm2" {sa213....}\r\n"vm3" {1231dsa....}'
    ...
    it 'should call runVBoxCmd with correct params', (done) ->
      model.getGuestStatus (err, result) ->
        expect(model.runVBoxCmd).toHaveBeenCalledWith 'list runningvms', jasmine.any(Function)
        done()

    it 'should return false on no machines found error', (done) ->
      model.getOnlyVirtualBoxDir.andCallFake (callback) ->
        process.nextTick -> callback new Error model.__get__ 'messageNoMachinesFound'
    ...
    it 'should fail error', (done) ->
      model.getOnlyVirtualBoxDir.andCallFake (callback) ->
        process.nextTick -> callback new Error 'some random error'

      model.getGuestStatus (err) ->
        expect(err).toBeTruthy();
        done()
